...
class IpbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ipb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ipb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ipb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ipb (date, title, link, content)"
